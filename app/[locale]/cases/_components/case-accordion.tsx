...
export default function CaseAccordion({
	...
	return (
		...
		>
			...
			<div
				ref={accordionRef}
				className='pl-[34px] rtl:pr-[34px] rtl:pl-0 space-y-6 relative'
			>
				{/* Indicator */}
				<div className='absolute left-0 rtl:right-0 rtl:left-auto top-0 w-2.5 rounded-full h-full bg-[#FAFAFA] overflow-hidden'>
					<div
						className='scroll-slider absolute left-0 rtl:right-0 rtl:left-auto top-0 right-0 rtl:left-0 rtl:right-auto h-[10%] bg-blue-500 rounded-full transition-all duration-500'
					...
					return (
						<div key={item.id}>
							<button
								...
								type='button'
								className='trigger-button flex items-start gap-4 text-left rtl:text-right'
							>
								<TypographyH4 className='text-blue-500 md:text-[28px] md:tracking-[0.28px]'>
									{locale === 'ar' ? toArabicNumerals(`${idx + 1}`) : `0${idx + 1}`}
								</TypographyH4>
							...
							>
								<div className='overflow-hidden'>
									<div className='pt-2.5 pl-[46px] rtl:pr-[46px] rtl:pl-0'>
