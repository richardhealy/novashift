...
export function Dialog({ isOpen, onClose, children, className, hideCloseButton = false }: DialogProps) {
	...
	// Close on escape key (only if close button is visible)
	useEffect(() => {
	...
	// Close on click outside (only if close button is visible)
	const handleClickOutside = (e: React.MouseEvent) => {
		if (!hideCloseButton && e.target === overlayRef.current) {
			onClose()
		}
	}
